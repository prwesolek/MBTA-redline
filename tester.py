...
import scipy
    ...
    #These write some basic stats into outfile for the various tests. These could probably be combined into 
...
def time(filename):
    ...
    #Run t-test 
    (stat,p)=scipy.stats.ttest_ind(dataS['ontime'],dataW['ontime'], equal_var=False)
    
    sign=0 # this variable checks to see if we need to close a file at the end.
    
    #These conditions check if the p-value from the t-test passes. If so, it writes a report to a txt file.
    #If the p-value is too large, it prints a failure notice to the command line.
    if p<=.05 and Smean>Wmean:
        outfile=open("test3-season-"+x+"-pass.txt",'w',)
        basicinfowritertime(outfile,Smean,Wmean,Sstd,Wstd,x)
        outfile.write("With 95% confidence the mean on-time performance in summer is better than the mean "+
                      " on-time performance in winter."+
                      " This test is for time-year "+x+".\n\n")
        print("Season test successful. See output txt file.")
        sign=sign+1
    elif p<=.05 and Smean<Wmean:
        outfile=open("test3-season-"+x+"-pass.txt",'w',)
        basicinfowritertime(outfile,Smean,Wmean,Sstd,Wstd,x)
        outfile.write("With 95% confidence the mean on-time performance in winter is better than the mean "+
                      " on-time performance in summer."+
                      " This test is for time-year "+x+".\n\n")
        print("Season test successful. See output txt file.")
        sign=sign+1
    elif p<=.1 and Smean>Wmean:
        outfile=open("test3-season-"+x+"-pass.txt",'w',)
        basicinfowritertime(outfile,Smean,Wmean,Sstd,Wstd,x)
        outfile.write("With 90% confidence the mean on-time performance in summer is better than the mean "+
                      " on-time performance in winter."+
                      " This test is for time-year "+x+".\n\n")
        print("Season test successful. See output txt file.")
        sign=sign+1
        ...
        outfile=open("test3-season-"+x+"-pass.txt",'w',)
        basicinfowritertime(outfile,Smean,Wmean,Sstd,Wstd,x)
        outfile.write("With 90% confidence the mean on-time performance in winter is better than the mean "+
                      " on-time performance in summer."+
                      " This test is for time-year "+x+".\n\n")
        print("Season test successful. See output txt file.")
        sign=sign+1
    else:
        print("Season test fails, because the p-value of",p," is too large.")
...
def precip(filename):
    ...
    for sorp in ["snow","precip"]:
        ...
        #T-test for noprecip on-time data verse precip on-time data
        (stat,p)=scipy.stats.ttest_ind(noprecip['ontime'],precip['ontime'], equal_var=False)
        
        #These conditions check if the p-value from the t-test passes. If so, it writes a report to a txt file.
        #If the p-value is too large, it prints a failure notice to the command line.
        if p<=.05 and precipmean>noprecipmean:
            outfile=open("test2-"+x+"-"+sorp+"-pass.txt",'w',)
            basicinfowriterprecip(outfile,precipmean,noprecipmean,precipstd,noprecipstd,sorp)
            outfile.write("With 95% confidence the mean on-time performance on days with "+sorp+
                          " is better than the mean on-time performance on days withOUT "+sorp+
                          ". This test is for time-year "+x+".\n\n")
            print("Test for precipitation type ",sorp," is successful. See output txt file.")
            sign=sign+1
        
        elif p<=.05 and precipmean<noprecipmean:
            outfile=open("test2-"+x+"-"+sorp+"-pass.txt",'w',)
            basicinfowriterprecip(outfile,precipmean,noprecipmean,precipstd,noprecipstd,sorp)
            outfile.write("With 95% confidence the mean on-time performance on days withOUT "+sorp+
                          " is better than the mean on-time performance on days with "+sorp+
                          ". This test is for time-year "+x+".\n\n")
            print("Test for precipitation type ",sorp," is successful. See output txt file.")
            sign=sign+1
        elif p<=.1 and precipmean>noprecipmean:
            outfile=open("test2-"+x+"-"+sorp+"-pass.txt",'w',)
            basicinfowriterprecip(outfile,precipmean,noprecipmean,precipstd,noprecipstd,sorp)
            outfile.write("With 90% confidence the mean on-time performance on days with "+sorp+
                          " is better than the mean on-time performance on days withOUT "+sorp+
                          ". This test is for time-year "+x+".\n\n")
            print("Test for precipitation type ",sorp," is successful. See output txt file.")
            sign=sign+1
            ...
            outfile=open("test2-"+x+"-"+sorp+"-pass.txt",'w',)
            basicinfowriterprecip(outfile,precipmean,noprecipmean,precipstd,noprecipstd,sorp)
            outfile.write("With 90% confidence the mean on-time performance on days withOUT "+sorp+
                          " is better than the mean on-time performance on days with "+sorp+
                          ". This test is for time-year "+x+".\n\n")
            print("Test for precipitation type ",sorp," is successful. See output txt file.")
            sign=sign+1
...
def temp(filename,temp):
    ...
    for ht in ['high','low']:   
        ...
        (stat,p)=scipy.stats.ttest_ind(aboveF['ontime'],belowF['ontime'], equal_var=False)
        
        if p<=.05 and aboveFmean>belowFmean:
            outfile=open("test1-"+x+"-"+ht+t+"-pass.txt",'w',)
            basicinfowritertemp(outfile,aboveFmean,belowFmean,aboveFstd,belowFstd,ht,t)
            outfile.write("With 95% confidence the mean on-time performance with "+ht+" temps BELOW "
                          +t+" is worse than the mean on-time performance with "+
                          ht+" temps ABOVE "+t+
                          ". This test is for time-year "+x+".\n\n")
            print("Temp test successful for "+ht+" temp data. See output txt file.")
            sign=sign+1
        elif p<=.05 and aboveFmean<belowFmean:
            outfile=open("test1-"+x+"-"+ht+t+"-pass.txt",'w',)
            basicinfowritertemp(outfile,aboveFmean,belowFmean,aboveFstd,belowFstd,ht,t)
            outfile.write("With 95% confidence the mean on-time performance with "+ht+" temps ABOVE "
                          +t+" is worse than the mean on-time performance with "+
                          ht+" temps BELOW "+str(temp)+". This test is for time-year "+x+".\n\n")
            print("Temp test successful for "+ht+" temp data. See output txt file.")
            sign=sign+1
        elif p<=.1 and aboveFmean>belowFmean:
            outfile=open("test1-"+x+"-"+ht+t+"-pass.txt",'w',)
            basicinfowritertemp(outfile,aboveFmean,belowFmean,aboveFstd,belowFstd,ht,t)
            outfile.write("With 90% confidence the mean on-time performance with "+ht+" temps BELOW "
                          +t+" is worse than the mean on-time performance with "+
                          ht+" temps ABOVE "+t+
                          ". This test is for time-year "+x+".\n\n")
            print("Temp test successful for "+ht+" temp data. See output txt file. ")
            ...
            outfile=open("test1-"+x+"-"+ht+t+"-pass.txt",'w',)
            basicinfowritertemp(outfile,aboveFmean,belowFmean,aboveFstd,belowFstd,ht,t)
            outfile.write("With 90% confidence the mean on-time performance with "+ht+" temps ABOVE "
                          +t+" is worse than the mean on-time performance with "+
                          ht+" temps BELOW "+str(temp)+". This test is for time-year "+x+".\n\n")
            print("Temp test successful for "+ht+" temp data. See output txt file.")
            sign=sign+1
        else:
            print("Temp test fails for ",ht," temp data, because the p-value of",p," is too large.")
